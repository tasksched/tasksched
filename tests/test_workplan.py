...
def test_workplan_sort_tasks():
    ...
    sorted_tasks = workplan.sorted_tasks(
...
def test_workplan_split_2():
    ...
    workplan = WorkPlan(
        Project(get_input_file("project_complete.yaml")),
        {"task3": 2, "task2": 2},
    )
...
def test_build_workplan_max_res():
    ...
    assert workplan.tasks[2].remaining == 0
    workplan_dict = workplan.as_dict()
    assert workplan_dict == get_input_file(
        "workplan_complete_max_resources.yaml"
    )
    str_workplan = yaml_dump(workplan_dict)
    assert str_workplan == get_input_file(
        "workplan_complete_max_resources.yaml", raw=True
    )
